import boto3, botocore, os, pathlib, sys, textwrap, argparse
...
def download_specific_file(filename: str, output_dir: str = None) -> None:
    """Download a specific file from S3 bucket.
    
    Args:
        filename: Name of the file to download (without the prefix)
        output_dir: Optional custom output directory. If not provided, uses LOCAL_DIR
    """
    if output_dir:
    ...
    try:
        ...
        s3.head_object(Bucket=BUCKET, Key=full_key, **({} if not EXTRA else EXTRA))
        
        # Download the file
        print(f"📥  {full_key}  →  {dest}")
        dl(BUCKET, full_key, dest)
        print(f"\n✅  Successfully downloaded {filename}")
        
    except botocore.exceptions.ClientError as e:
        if e.response["Error"]["Code"] == "404":
            print(f"❌  Error: File '{filename}' not found in bucket {BUCKET}/{PREFIX}")
        else:
            print(f"❌  Error accessing {full_key}: {e}")

def main():
    parser = argparse.ArgumentParser(description="Download a specific file from S3 bucket")
    parser.add_argument("filename", help="Name of the file to download (without the prefix)")
    parser.add_argument("-o", "--output", help="Custom output directory (optional)")
    
    args = parser.parse_args()
    
    try:
        download_specific_file(args.filename, args.output)
